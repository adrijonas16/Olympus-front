...
const SelectClient: React.FC<SelectClientProps> = ({
  ...
  const [clienteSeleccionado, setClienteSeleccionado] = useState<ClientePotencial | null>(null);
  ...
  const handleConfirmClient = () => {
    if (clienteSeleccionado) {
      if (onSelectClient) {
        ...
        navigate('/leads/CreateOpportunity', { state: { client: clienteSeleccionado } });
      }
    }
  };
  ...
  const filteredClientes = searchText.trim() === ''
    ? clientes
    : clientes.filter(cliente => {
        const nombreCompleto = `${cliente.persona.nombres} ${cliente.persona.apellidos}`.toLowerCase();
    ...
    >
      <div className="select-client-container">
        ...
        <AutoComplete
          ...
          notFoundContent={loading ? 'Cargando...' : 'No se encontraron clientes'}
          filterOption={false}
            ...
            <div className="selected-client-header">
              <div style={{ flex: 1 }}>
                <h3 className="selected-client-name">
                  {clienteSeleccionado.persona.nombres} {clienteSeleccionado.persona.apellidos}
                </h3>
              </div>
              <Button
                type="text"
                ...
                style={{ color: '#8c8c8c', padding: '0', minWidth: '20px', height: '20px' }}
              />
            </div>
            <div className="selected-client-details">
              <p className="selected-client-detail">
                <strong>Celular:</strong>
                {clienteSeleccionado.persona.prefijoPaisCelular} {clienteSeleccionado.persona.celular}
              </p>
              <p className="selected-client-detail">
